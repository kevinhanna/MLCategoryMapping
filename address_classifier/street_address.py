...
class StreetAddress:
    ...
    def compile_address_types(self):
        retval = {au.STREET_NUMBER: [], au.STREET_NAME: [], au.STREET_TYPE: [], au.SUITE: [], au.BUILDING_NUMBER: [], au.HIGHWAY_NAME: [], au.HIGHWAY_NUMBER: []}
        ...
        for type, token in zip(self.street_address_types, self.street_address_tokens):
            ...
            elif type == au.HIGHWAY_NAME:
                retval[au.HIGHWAY_NAME].append(token)
            elif type == au.HIGHWAY_NUMBER:
                retval[au.HIGHWAY_NUMBER].append(token)
    ...
    def match_street_address(self, target_street_address):
        ...
        match_details[au.STREET_NAME] = self.match_street_name(source[au.STREET_NAME], target[au.STREET_NAME])
        match_details[au.STREET_TYPE] = self.match_street_type(source[au.STREET_TYPE], target[au.STREET_TYPE])

        match_details[au.SUITE] = self.match_suite(source[au.SUITE], target[au.SUITE])
        match_details[au.BUILDING_NUMBER] = self.match_building_number(source[au.BUILDING_NUMBER], target[au.BUILDING_NUMBER])
        #TODO HIGHWAY_NAME and _NUMBER
        #TODO STREET_DIRECTION
    ...
    def match_street_type(self, a, b):
        ...
        #TODO account for <> 1 street type
        for type in self.STREET_NORMALIZATIONS:
            for synonyms in type:
                if (a[0] in synonyms and b[0] in synonyms):
                    return True
